...
## Normalize
As this point data is normalized as well, meaning that numerical variables are transformed to a homogeneous scale of values. The normalizing uses train dataset to be fitted to, but then after using statistical properties of only train, it changes both train and test dataset. Otherwise, if statistical properties of test is used, data leakage occurs and the models can cheat by footprint of test data on normalized train data.
