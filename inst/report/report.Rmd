...
1. Many machine learning algorithms cannot handle missing values, 2. Missing values introduce bias or lead to inaccurate conclusions if the missing data is not missing at random. 
...
```{r, echo=FALSE, warning=FALSE, fig.dim = c(7,2.5)}
...
gridExtra::grid.arrange(p1, p2, ncol = 2, top = 0.5, left=0.5)
```
...
matrics_logit <- c(accuracy_logit, precision_logit, recall_logit)
metrics_rf <- c(accuracy_rf, precision_rf, recall_rf)
df_eval <- data.frame(metrics_list, matrics_logit, metrics_rf)
```
...
Precision & `r precision_logit` & `r precision_logit` \\
Recall & `r recall_logit` & `r recall_logit` \\
\hline
\end{tabular}
...
roc_rf <- pROC::roc(actual ~ pred_proba_glm, print.auc=FALSE)
...
```{r, echo=FALSE, warning=FALSE, fig.dim = c(5,2.5)}
options(repr.plot.width=2, repr.plot.height=2)
...
gridExtra::grid.arrange(p1, p2, ncol = 2, top = 0.5, right=1)
```
