...
a customer's is approved or not. The \texttt{aiinsurance} R package
...
main steps of this work in a single command, and an interactive app to
display the performance of the models. Using package's functions, two
machine learning models, i.e., logistic regression and random forest are
implemented to predict the outcomes based on solely the historical data.
The results indicate promising prediction power with various evaluation
metrics (e.g., accuracy, precision, recall, etc.) of over 80 percent. In
addition to prediction, informative insights from the dataset and models
have been drawn, e.g., by dint of the random forest model, the variables
of the dataset that perform effective role in the prediction become
evident.
\end{abstract}
...
discrete classes, in our case, status of claims. The latter model is a
...
policyholder. In section \ref{subsec:modelsintro}, the machine learning
models, logistic regression \ref{subsubsec:logit}, and random forest
\ref{subsubsec:rf} are briefly explained. As a contribution of this
...
in our case, a car accident. The insurance company either accepts or
rejects the claim. If it is approved, the insurance company will issue
payment to the insured or an approved interested party on behalf of the
insured.

Predicting the outcome of claims be utilized to better understand the
customer strata and incorporate the findings throughout the insurance
policy enrollment (including the underwriting and approval or rejection
stages), triage claims and automate where possible, gradually obviating
the need for human interaction, and optimize the entire insurance policy
enrollment process flow. \cite{claim}

\hypertarget{models}{%
\subsection{\texorpdfstring{Models
\label{subsec:modelsintro}}{Models }}\label{models}}

\hypertarget{logistic-regression}{%
\subsubsection{\texorpdfstring{Logistic Regression
\label{subsubsec:logit}}{Logistic Regression }}\label{logistic-regression}}

\hypertarget{random-forest}{%
\subsubsection{\texorpdfstring{Random Forest
\label{subsubsec:rf}}{Random Forest }}\label{random-forest}}
...
\section{\texorpdfstring{Implementation and Results
\label{sec:impl}}{Implementation and Results }}\label{implementation-and-results}}
...
\subsection{\texorpdfstring{Dataset Analysis
\label{subsec:data}}{Dataset Analysis }}\label{dataset-analysis}}
