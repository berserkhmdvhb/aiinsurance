...
policyholders, among which the variable of interest is the outcome of
insurance, indicating whether a customer has claimed his or not. The
...
e.g., a pipeline that performs the main steps of this work with a single
command, and an interactive app to display the performance of the
models. Using package's functions, two machine learning models, i.e.,
logistic regression and random forest are implemented to predict the
outcomes based on solely the historical data. The results indicate
promising prediction power with various evaluation metrics (e.g.,
accuracy, precision, recall, etc.) of over 80 percent. In addition to
prediction, informative insights from the dataset and models have been
drawn, e.g., by dint of the random forest model, the variables of the
dataset that perform effective role in the prediction become evident.
\end{abstract}
...
In this work, both a classical linear model and a modern nonlinear model
is used and compared for predicting policyholders' claims. The former is
logistic regression, which is an example of GLMs, accommodated to
classification setting, i.e., for predicting discrete classes, in our
case, ``outcome'' of insurance that indicates whether a customer has
claimed his loan or not.

The remainder of this work is organized as the following: In section
\ref{concepts}, main concepts included in the work are explained. In
\ref{claim}, the concept of insurance claim and how it interacts among
an insurer and a policyholder is explained, and in \ref{models}, the
machine learning models, logistic regression \ref{} \cite{package}

\hypertarget{preliminary-concepts}{%
\section{\texorpdfstring{Preliminary Concepts
\label{concepts}}{Preliminary Concepts }}\label{preliminary-concepts}}

\hypertarget{car-insurance-claims-claim}{%
\subsection{\texorpdfstring{Car Insurance Claims \{claim\}\\
}{Car Insurance Claims \{claim\} }}\label{car-insurance-claims-claim}}
